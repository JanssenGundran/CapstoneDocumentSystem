...
export class CertificateDownloadComponent implements OnInit {
 ...
 ngOnInit() {
  ...
  // If localStorage empty, create a test user
  if (!user.role) {
    console.warn('No user found, faking local user for testing...');
    this.userRole = 'certificate_editor';
    localStorage.setItem('user', JSON.stringify({ email: 'test@example.com', role: this.userRole }));
  } else {
    this.userRole = (user.role || '').toLowerCase();
  }

  // Skip accessDenied check entirely for testing
